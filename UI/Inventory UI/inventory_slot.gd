...
@onready var texture_rect: TextureRect = $NinePatchRect/MenuButton/CenterContainer/ItemTexture
...
func add_item(item: InventoryItem):
	if item.slot_type != "NotEquipable":
		...
		popup_menu.set_item_text(0, "Equip to " + equip_slot_name.capitalize())
...
func toggle_button_selected_variation(button_selected: bool):
	on_click_button.theme_type_variation = "selected" if button_selected else ""


func show_price_tag(price: int):
	price_label.text = str(price) + " ryo"
	price_label.show()
