...
const INVENTORY_SLOT_SCENE = preload("res://UI/Inventory UI/inventory_slot.tscn")
...
func toggle():
			visible = !visible
			if visible:
				if Global.get_currency() > 1:
					score_label.text = 'GOLD: ' + str(Global.get_currency())
				else:
					score_label.text = "Fight inmates to earn currency"
